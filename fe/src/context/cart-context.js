...
export function CartProvider({ children }) {
  ...
  const fetchCartItems = async (currentToken) => {
    ...
    try {
      const res = await fetch("http://192.168.1.101:8080/VuaCaQPQ/cart/items", {
        headers: {
  ...
  const addToCartAPI = async (product, quantity, authToken) => {
    try {
      await fetch("http://192.168.1.101:8080/VuaCaQPQ/cart/items", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
  ...
  const removeFromCart = async (productId, id) => {
    ...
    try {
      const res = await fetch(`http://192.168.1.101:8080/VuaCaQPQ/cart/items/${id}`, {
        method: "DELETE",
